...
export class AboutService {
  ...
  updateFeedback(feedback:Feedback){
    return this.http.put<Feedback>(this.baseUrl + 'about/feed/' + feedback.id, feedback).pipe(
      map(() =>{}
      )
  )}
  ...
  uploadFeedPhoto(file, feedId:number):Observable<any>{
    const formData = new FormData();
    formData.append("file", file, file.name);
    return this.http.post(this.baseUrl + 'about/feed/add-photo/' + feedId, formData);
  }

  registerFeedback(model:any){
    return this.http.post<Feedback>(this.baseUrl + 'about/feed', model).pipe(
      map((feedback) => {
  ...
  deleteFeedback(id:number) {
    return this.http.delete(this.baseUrl + 'about/feed/delete/' + id, {responseType: 'text'})
  }
}
